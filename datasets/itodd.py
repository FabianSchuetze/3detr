...
class ItoddDatasetConfig(object):
    def __init__(self):
        self.num_semcls = 10
...
class ItoddDetectionDataset(Dataset):
    def __init__(
        self,
        dataset_config,
        split_set="train",
        ...
        if split_set in ["train", "val"]:
            scan_names = np.loadtxt(os.path.join(root_dir, 'splits', 'train.txt'),
                            dtype=int)
            self.scan_names = ["%07d"%(i) for i in scan_names]
    ...
    def __getitem__(self, idx):
        ...
        point_cloud = np.load(os.path.join(self.image_path, scan_name + ".npz"))["arr_0"]  # Nx6
        bboxes = np.load(os.path.join(self.gt_path, scan_name + ".npy"))  # Nx6
        #change to m
        bboxes = bboxes / 1000
        ...
        box_centers = bboxes.astype(np.float32)
